...
# ### Try Using these Amazing kernels 
...
# - https://www.kaggle.com/nz0722/reasoning-some-text-cleaning-not-work
# 
# ## <center> <font color='red'> And avoid this kernel as it Might Have unseen bugs (due to my little NLP Knowledge)</font> </center>
# 
...
# - https://www.kaggle.com/adityaecdrid/annoying-artgor-regex-primer-xd (for regex maybe)
# 
# >> Basically in this kernel, I blended from the above two kernel's pre-processing as well as extra things to handle the current dataset as well....
# and what you are seeing is finished work haha (took me around ~$14$ hours and around 24 kernel versions (private) to raise the vocab coverage from $50%$ to $65+$ :)
# 
# - PS The preprocessing helped me reach **.93xxx** and there's still scope for more :)
# 
# - Let me know my shortcomings :)
# 
# - Also US People especially might enjoy this comp more :) found lot of President Trump's things hahah...
# 
# - ** Be CareFul; There might be Few BUGS!**
# ## <center> <font color='red'> Take What You Need, Too Much doesn't help </font> </center>
# 
# Thanks
...
train = pd.read_csv("../input/jigsaw-unintended-bias-in-toxicity-classification/train.csv", usecols=['comment_text'])
...
# - the code(mapping) is hidden in 2 cells below
...
#  - in the hidden code, i have expanded the corrections (click to see it or else ignore this)
...
test['comment_text'] = test['comment_text'].progress_apply(lambda x: correct_spelling(x, mispell_dict))
test["comment_text"] = test["comment_text"].progress_apply(lambda x: clean_text(x))
test['comment_text'] = test['comment_text'].progress_apply(lambda x: clean_number(x)) #incase
...
print('Default Embedding fasttext oov rate:', oov_fasttext['oov_rate'])

# In[ ]:


oov_paragram['oov_words'][:50] #realDonaldTrump, Trumpland, Trumpettes, TheDonald....
...
oov_fasttext['oov_words'][:50]

# - Thanks For Passing By And Happy Kaggling
